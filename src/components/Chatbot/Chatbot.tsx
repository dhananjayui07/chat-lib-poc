...
const ChatBot: React.FC<ChatBotProps> = ({ closeChatBot }) => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: '"Do you have an existing phone number you would like to keep?"',
      sender: "bot",
    },
  ...
  return (
    <>
      <div
        className={`${styles.mainWrapBlue} rounded-top-4 d-flex flex-column`}
      >
        {/* window header */}
        ...
        <Tab.Container defaultActiveKey="tabScript">
          ...
          <div className={`${styles.navWrap} mb-3 mx-3 p-2 rounded`}>
            <Nav
              variant="pills"
              className={`${styles.buttonsNavHome} gap-1 justify-content-between`}
            >
              <Nav.Item>
                <Nav.Link
          ...
          <div
            className={`${styles.promptTabContainer} d-flex flex-column flex-grow-1`}
          >
            <Tab.Content className="flex-grow-1">
              <Tab.Pane eventKey="tabScript" className="h-100">
                ...
                <div className="position-relative h-100">
                  <div
                    className={`${styles.chatPanelWrap} h-100 px-3`}
                    ref={messagesContainerRef}
                  >
                    <div className={`${styles.stepContent} mb-3`}>
                      <h4 className="fs-6 fw-bold">
                        Step 3: Phone Configuration
                      </h4>
                      <p className="m-0">
                        In this step, you can configure the phone line with
                        calling features and offer phone number porting or
                        search for a prefered number for the customer
                      </p>
                    </div>
                    {/* chat messages */}
                      ...
                      <div
                        ...
                        className={`${
                          message.sender === "you"
                            ? styles.sent
                            : styles.received
                        } mb-3 p-3 shadow-sm ${styles.chatBubble}`}
                      >
                        {message.text}
                      </div>
                    ))}
                  ...
                  <div className="position-absolute bottom-0 end-0 start-0 px-3 py-2 bg-info d-flex gap-2">
                    ...
                    >
                      <span className="material-icons text-dark fs-5">
                        send
                      </span>
                    </Button>
                  </div>
